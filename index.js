...
// Console output for total number of months
console.log("Total months: " + finances.length);

// Used reduce methof on the finances array to sum app all the P&L values
totalPL = finances.reduce((a, b) => a + b[1], 0);

// Console output for total P&L
console.log("Total: $" + totalPL);

// Creation of a second array containing all the changes in P&L month by month over the period
var changes = [];
for (var i = 1; i < finances.length; i++) {
    changes.push([finances[i][0], (finances[i][1] - finances[i - 1][1])]);
}
...
// Initializing variable for greatest increase in profits at 0
var biggestProfit = [0, 0];

// Looping through the changes array to find the biggest profit increase 
...
// Looping through the changes array to find the biggest profit decrease 
for (var i = 0; i < changes.length; i++) {

    if (changes[i][1] < biggestLoss[1]) {
        biggestLoss = [changes[i][0], changes[i][1]];
    }
}
